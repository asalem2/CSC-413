...
public class Parser {
    ...
    private EnumSet<Tokens> UnaryOps =
            EnumSet.of(Tokens.Invert,Tokens.Minus);
    /**
...
public AST rBlock() throws SyntaxError {
    ...
    boolean startingDecl() {
        ...
        if (isNextTok(Tokens.Int) || isNextTok(Tokens.BOOLean) || isNextTok(Tokens.Float) || isNextTok(Tokens.Char)) {
    ...
    boolean startingStatement() {
        ...
        if (isNextTok(Tokens.If) || isNextTok(Tokens.While) || isNextTok(Tokens.Return)
                ...
                || isNextTok(Tokens.LeftBrace) || isNextTok(Tokens.Identifier) || isNextTok(Tokens.Do)) {
 ...
 *  type  ->  'float'
 *  </pre>
 *  @return either the intType, boolType, floatType, charType tree
 *  @exception SyntaxError - thrown for any syntax error
*/
...
*/
    public AST rFactor() throws SyntaxError {
        ...
        if (isNextTok(Tokens.Floating)) {  //  -> <float>
            t = new FloatTree(currentToken);
            scan();
            return t;
        }
        if (isNextTok(Tokens.Character)) {  //  -> <char>
...
*/        
    ...
    private AST getUnarytOperTree() {
        Tokens kind = currentToken.getKind();
        if (UnaryOps.contains(kind)) {
            AST t = new UnaryOpTree(currentToken);
            scan();
            return t;
        } else {
            return null;
